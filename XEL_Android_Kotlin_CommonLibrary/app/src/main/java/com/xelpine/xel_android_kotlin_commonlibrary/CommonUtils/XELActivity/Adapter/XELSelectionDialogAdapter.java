...
public class XELSelectionDialogAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>
{
    // 기본데이터
    ...
    public XELSelectionDialogAdapter(ArrayList<? extends XELCommonSelectionInterface> contactList, int selectedPosition)
    {
        this.launchList = contactList;
...
//        XELLogUtil.i(XELGlobalDefine.TAG, "===== LaunchAdapter onBindViewHolder position = " + position);
        ...
        switch (contactViewHolder.getItemViewType())
        {
            case DEFAULT_VIEWTYPE:
                ...
                viewHolder_default.xel_row_selectionDialog_tv_title.setText(model.getName() + " (" + model.getCode() + ")");

                // onClick
                viewHolder_default.xel_row_selectionDialog_cl_mainLayout.setOnClickListener(new View.OnClickListener()
                {
                    @Override
...
//        XELLogUtil.i(XELGlobalDefine.TAG, "===== LaunchAdapter onCreateViewHolder getMessageType : " + viewType);
        ...
        {
            case DEFAULT_VIEWTYPE:
                itemView = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.xel_row_selectiongdialog, viewGroup, false);
...
//        return launchList.get(position).getMessageType();
    ...
    {
        ConstraintLayout xel_row_selectionDialog_cl_mainLayout;
        TextView xel_row_selectionDialog_tv_title;

        public MainMenuDtoViewHolder (View v)
        {
            super(v);

            xel_row_selectionDialog_cl_mainLayout = (ConstraintLayout) v.findViewById(R.id.xel_row_selectionDialog_cl_mainLayout);
            xel_row_selectionDialog_tv_title = (TextView) v.findViewById(R.id.xel_row_selectionDialog_tv_title);
        }
    }
};