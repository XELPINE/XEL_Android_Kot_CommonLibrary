...
class XELSelectionDialogAdapter(
...
//        XELLogUtil.i(XELGlobalDefine.TAG, "===== LaunchAdapter onBindViewHolder position = " + position);
        ...
        when (contactViewHolder!!.itemViewType) {
            DEFAULT_VIEWTYPE -> {
                ...
                viewHolder_default!!.xel_row_selectionDialog_tv_title.setText(
                    model.Name().toString() + " (" + model.Code() + ")"
                )

                // onClick
                viewHolder_default.xel_row_selectionDialog_cl_mainLayout.setOnClickListener { view ->
                    if (itemClick != null) {
        ...
        when (viewType) {
            DEFAULT_VIEWTYPE -> {
                itemView = LayoutInflater.from(viewGroup.context)
                    .inflate(R.layout.xel_row_selectiongdialog, viewGroup, false)
...
//        return launchList.get(position).getMessageType();
    ...
    inner class MainMenuDtoViewHolder(v: View?) : RecyclerView.ViewHolder(
        v!!
    ) {
        var xel_row_selectionDialog_cl_mainLayout: ConstraintLayout
        var xel_row_selectionDialog_tv_title: TextView

        init {
            xel_row_selectionDialog_cl_mainLayout =
                v!!.findViewById<View>(R.id.xel_row_selectionDialog_cl_mainLayout) as ConstraintLayout
            xel_row_selectionDialog_tv_title =
                v.findViewById<View>(R.id.xel_row_selectionDialog_tv_title) as TextView
        }
    }
